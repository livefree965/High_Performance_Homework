...
int main(int argc, char *argv[])
{
	...
	*/
	...
	int each_nodecal = cell_nums / comm_size;
	...
	for (int i = 0; i < mat_size[0]; i++)
	{
		for (int j = 0; j < mat_size[1]; j++)
		{
			...
			if (acculate >= each_nodecal) {
				endpos = i + 1;
				parts.push_back(pair<int, int>(begpos, endpos));
				begpos = endpos;
				acculate = 0;
			}
		}
	}
	parts.push_back(pair<int, int>(begpos, mat_size[0]));
	int part = mat_size[0] / comm_size;
	if (my_rank != 0) {
		for (int row = parts[my_rank].first; row < parts[my_rank].second; row++) {
			for (int col = 0; col < vec_size[1]; col++) {
				for (int inner = 0; inner < mat_size[1]; inner++) {
					res[row][col] += mat_data[row][inner] * vec_data[inner][col];
				}
	...
	else
	{
		...
		{
			printf("Core%d for row :%d-%d\n", i, parts[i].first, parts[i].second);
		}
		//show_mat(mat_a);
		//show_mat(mat_b);
		for (int i = 1; i < comm_size; i++)
		{
			for (int row = parts[i].first; row < parts[i].second; row++) {
